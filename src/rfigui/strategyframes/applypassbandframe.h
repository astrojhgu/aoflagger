...
#include "../../strategy/actions/applypassbandaction.h"

#include "../editstrategywindow.h"

class ApplyPassbandFrame : public Gtk::Frame {
	public:
		ApplyPassbandFrame(rfiStrategy::ApplyPassbandAction& action, EditStrategyWindow& editStrategyWindow)
		: Gtk::Frame("Apply passband"),
		...
		_applyButton("Apply")
		{
			...
			_filenameBrowseButton.signal_clicked().connect(sigc::mem_fun(*this, &ApplyPassbandFrame::onBrowseClicked));

			_buttonBox.pack_start(_applyButton);
			_applyButton.signal_clicked().connect(sigc::mem_fun(*this, &ApplyPassbandFrame::onApplyClicked));
	...
	private:
		...
		rfiStrategy::ApplyPassbandAction &_action;
		...
		{
			Gtk::FileChooserDialog dialog(_editStrategyWindow, "Select passband file");
